...
export default class ReporteCuentasCobrar extends React.Component {
  ...
  sumaCuentasPorCobrar(num_semana){
    let suma=0
    switch (num_semana){
      case 1:
        this.state.array.map((value, index) => {
          if(value.mes===this.state.mes && value.tipo_registro==="Cuentas por Cobrar")
    ...
    <div>
        ...
        <table className="table table-bordered">
          ...
          <tbody>
            ...
            <tr>
              <td>Total:</td>
              <td>{"$ "+this.sumaCuentasPorCobrar(1)}</td>
              <td>{"$ "+this.sumaCuentasPorCobrar(2)}</td>
              <td>{"$ "+this.sumaCuentasPorCobrar(3)}</td>
              <td>{"$ "+this.sumaCuentasPorCobrar(4)}</td>
              <td>{"$ "+this.sumaCuentasPorCobrar(4)}</td>
            </tr>
          </tbody>
        </table>
