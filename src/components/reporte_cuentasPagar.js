...
export default class ReporteCuentasPagar extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      ObjetoIndicadores: [],
  ...
  sumaCuentasPorPagar(num_semana){
    let suma=0
    switch (num_semana){
      case 1:
        this.state.array.map((value, index) => {
          if(value.mes===this.state.mes && value.tipo_registro==="Cuentas por Pagar")
    ...
    <div>
        ...
        <table className="table table-bordered">
          ...
          <tbody>
            ...
            <tr>
              <td>Total:</td>
              <td>{"$ "+this.sumaCuentasPorPagar(1)}</td>
              <td>{"$ "+this.sumaCuentasPorPagar(2)}</td>
              <td>{"$ "+this.sumaCuentasPorPagar(3)}</td>
              <td>{"$ "+this.sumaCuentasPorPagar(4)}</td>
              <td>{"$ "+this.sumaCuentasPorPagar(4)}</td>
            </tr>
          </tbody>
        </table>
